...
import { backupCodesManager } from '@/lib/supabase-backup-codes'
  ...
  const onSubmitClicked = async (e: React.FormEvent) => {
    ...
    try {
      if (useBackupCode) {
        ...
        const isValid = await backupCodesManager.validateBackupCode(backupCode.trim())
        if (isValid) {
          // Backup code is valid, refresh session and redirect
      ...
      <div className="w-full max-w-[420px] rounded-[24px] bg-white p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] ring-1 ring-gray-900/5">
        <div className="mb-8 text-center flex flex-col items-center">
          ...
          <p className="mt-2 text-[14px] text-gray-500 leading-relaxed px-4">
            {useBackupCode 
              ? 'Enter an 8-character backup code to access your account.'
        ...
        <form onSubmit={onSubmitClicked} className="flex flex-col gap-5">
          {useBackupCode ? (
            <div className="flex flex-col gap-2">
              <label className="text-[13px] font-semibold text-gray-700">Backup Code</label>
              <input
                type="text"
                required
                maxLength={8}
                value={backupCode}
                onChange={(e) => setBackupCode(e.target.value.replace(/[^A-Z0-9]/g, ''))}
                placeholder="Enter backup code"
                className="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3.5 text-[16px] font-mono tracking-wide text-center outline-none focus:bg-white focus:border-blue-600 focus:ring-4 focus:ring-blue-600/10 transition-all placeholder:text-gray-300"
              />
            </div>
          ) : (
            <div className="flex flex-col gap-2">
              <label className="text-[13px] font-semibold text-gray-700">Authenticator Code</label>
              <input
                type="text"
                required
                maxLength={6}
          ...
          <button
            type="submit"
            disabled={loading || (useBackupCode ? backupCode.length !== 8 : verifyCode.length !== 6)}
